...
public class SparseSet<T> where T : struct, IComponent
{
    ...
    public SparseSet(int initialCapacity = 1000, int growCapacity = 1000, uint maxEntityId = 100000, uint pageSize = 1024)
    {
    ...
    public ReadOnlySpan<T> GetAll()
    {
        ...
        for (var i = 0; i < denseSpan.Length; i++)
        {
            components[i] = denseSpan[i].Component;
        }
    ...
    public void RemoveRange(ReadOnlySpan<Entity> entities)
    {
        ...
        for (var i = 0; i < entities.Length; i++)
        {
            ...
            if (entityId > _maxEntityId) continue;
            
            var idx = GetSparseIndex(entityId);
            if ((uint)idx < (uint)Count)
            {
                indicesToRemove[validCount++] = idx;
            }
        }
    ...
    private void EnsureCapacity(int requiredCapacity)
    {
        ...
        if (Count > 0)
        {
            Array.Copy(_dense, 0, newDense, 0, Count);
        }
